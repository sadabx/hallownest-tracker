import { GROUPS, HK, REGIONS, WIKI_ROOT, escapeHTML, statValue, stripMarkup, summary } from "../app/tracker-model.js";
...
function renderEntryCard(item) {
  ...
  const glyph = { main: "♠", essentials: "◆", journal: "✦", collectibles: "◇", world: "⌖", godhome: "♛" }[item.group] || "•";
  const icon = itemIconFor(item);
  const artwork = icon ? `<img src="${icon}" alt="" loading="lazy">` : glyph;
  return `<article class="check-card status-${item.status}" data-entry-details="${item.id}" tabindex="0">
    ...
    <span class="check-section">${escapeHTML(item.section)}</span>
    <div class="check-art" aria-hidden="true">${artwork}</div>
    <div class="check-copy"><h3>${escapeHTML(item.name)}</h3><p class="spoiler-copy ${state.spoilers ? "revealed" : ""}">${state.spoilers ? escapeHTML(location) : "Location hidden"}</p></div>
...
function renderProgress(entries) {
  ...
  const body = groupEntries.map(([groupKey, group]) => {
    ...
    return `<section class="progress-group" data-group-name="${escapeHTML(group.label)}"><h2 class="progress-group-banner">${escapeHTML(group.label)}</h2>${sectionBody}</section>`;
  }).join("");
  ...
  document.querySelector("#progress-view").innerHTML = `
    ...
    <div class="hero-stats progress-summary">
      ...
      <article><span>Tracked:</span><strong>${HK.saveAnalyzed ? stats.complete : 0}/${stats.actionable}</strong></article>
      <article><span>True Completion:</span><strong>${HK.saveAnalyzed ? trueCompletion.spoiler : "0%"}</strong></article>
    </div>
...
function renderProgressTOC() {
  ...
  toc.innerHTML = `<nav>${groups.map(group => {
    ...
    return `<div class="toc-group"><strong>• ${escapeHTML(group.dataset.groupName)}</strong>${sections.map(section => {
